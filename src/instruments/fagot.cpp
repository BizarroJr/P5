...
Fagot::Fagot(const std::string &param): adsr(SamplingRate, param) {
  bActive = false;
  x.resize(BSIZE);
...
void Fagot::command(long cmd, long note, long vel) {
...
const vector<float> & Fagot::synthesize() {  
  if (not adsr.active()) {
    x.assign(x.size(), 0);
    bActive = false;
    return x;
