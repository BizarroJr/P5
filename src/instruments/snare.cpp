...
Snare::Snare(const std::string &param): adsr(SamplingRate, param) {
  bActive = false;
  x.resize(BSIZE);
...
void Snare::command(long cmd, long note, long vel) {
...
const vector<float> & Snare::synthesize() {  
  if (not adsr.active()) {
    x.assign(x.size(), 0);
    bActive = false;
    return x;
